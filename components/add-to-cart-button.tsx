...
export default function AddToCartButton({ product, sizes }: AddToCartButtonProps) {
	...
	const [selectedSizeId, setSelectedSizeId] = useState<string | null>(
		sizes && sizes.length > 0 ? sizes[0].id : null
	)
						...
						{sizes.map((size) => {
							...
							return (
								<button
									...
									className={`p-4 border rounded-md transition-all text-left ${isSizeOutOfStock
								...
								>
									<div className="flex flex-col">
										<div className="flex items-center justify-between mb-1">
											<span className={`text-sm font-medium ${isSelected ? 'text-emerald-700' : 'text-gray-900'
												}`}>
												{size.size_grams}g
											</span>
											{isSelected && (
										...
										<span className={`text-lg font-serif mt-1 ${isSelected ? 'text-emerald-900' : 'text-gray-900'
											}`}>
											KSh {Number(size.price).toFixed(2)}
										</span>
										<span className={`text-xs mt-1 ${isSizeOutOfStock
